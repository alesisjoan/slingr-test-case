    ...
    <div className="App">
      <div className="row">
        ...
        <h5  style={{margin: 5}}>Last expressions</h5>
          <LastExpressionsComponent lastExpressions={lastExpressions} setLastExpressions={setLastExpressions}/>
        </div>
        </div>
       
      </div>
      <div className="column2">
        <SwitchApiComponent toggleAPI={toggleAPI} setToggleAPI={handleToggleAPI} 
        toggleMethod={toggleMethod} setToggleMethod={handleToggleMethod} />
        <br/>
      ...
      <div className="column3">
                <div className="calculator-body">
                    ...
                    <ExpressionComponent mathExpression={mathExpression}/><br/>
...
const LastExpressionsComponent = ({lastExpressions, setLastExpressions}) => {
  fetch(LAST_EXPRESSIONS).then((response) => response.text())
  .then((responseJson) => {setLastExpressions(responseJson)});
  return <pre style={{
...
const DigitsComponent = ({digits, onchange}) => {
  return (<div>
        <input name="digits" type="number" min="0" max="100" value={digits} onChange={onchange} />
...
const ExpressionComponent = ({mathExpression}) => {
  return (
        <div className="expression">
            <p className="legend">Expression</p><p> {mathExpression}</p>
        </div>
  )
}
...
const SwitchApiComponent = ({toggleAPI, setToggleAPI, toggleMethod, setToggleMethod}) => {
  return (
    <div>
...
const NumPadComponent = ({onClickNumPad}) => {
  return (
    <div className="button">
        ...
        <button className="buttonHalf" name="(" onClick={e => onClickNumPad(e.target.name)}>(</button>
          <button className="buttonHalf" name=")" onClick={e => onClickNumPad(e.target.name)}>)</button>
          <button name="sqrt(" onClick={e => onClickNumPad(e.target.name)}>sqrt</button>
        <button name="back" onClick={e => onClickNumPad(e.target.name)}>&#8592;</button>
        
        <button name="C" onClick={e => onClickNumPad(e.target.name)}>C</button><br/>


        <button name="1" onClick={e => onClickNumPad(e.target.name)}>1</button>
        <button name="2" onClick={e => onClickNumPad(e.target.name)}>2</button>
        <button name="3" onClick={e => onClickNumPad(e.target.name)}>3</button>
        <button name="+" onClick={e => onClickNumPad(e.target.name)}>+</button><br/>


        <button name="4" onClick={e => onClickNumPad(e.target.name)}>4</button>
        <button name="5" onClick={e => onClickNumPad(e.target.name)}>5</button>
        <button name="6" onClick={e => onClickNumPad(e.target.name)}>6</button>
        <button name="-" onClick={e => onClickNumPad(e.target.name)}>-</button><br/>

        <button name="7" onClick={e => onClickNumPad(e.target.name)}>7</button>
        <button name="8" onClick={e => onClickNumPad(e.target.name)}>8</button>
        <button name="9" onClick={e => onClickNumPad(e.target.name)}>9</button>
        <button name="*" onClick={e => onClickNumPad(e.target.name)}>x</button>
        
        <br/>



        <button name="." onClick={e => onClickNumPad(e.target.name)}>.</button>
        <button name="0" onClick={e => onClickNumPad(e.target.name)}>0</button>
        <button name="=" onClick={e => onClickNumPad(e.target.name)}>=</button>
        <button name="/" onClick={e => onClickNumPad(e.target.name)}>÷</button><br/>
    </div>
);
